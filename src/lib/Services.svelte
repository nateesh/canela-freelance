<script>

  import "$lib/global.css"; 

</script>

<div class="xl:col-span-2 mt-10">
  

  <div class="mb-10 pl-3">
    <ul class="service-list list-none text-lg grid xs:grid-cols-2 grid-cols-1 gap-3 whitespace-pre-wrap">
      <li>Vector and raster <span class="text-lime-700/80">Spatial analysis.</span></li>
      <li><span class="text-lime-700/80">Database management</span> (PostgreSQL, SpatiaLite).</li>
      <li><span class="text-lime-700/80">Enterprise</span> GIS Training - ArcGIS.</li>
      <li><span class="text-lime-700/80">Open-source</span> GIS Training - QGIS.</li>
      <li>Custom <span class="text-lime-700/80">Geo-spatial programming</span> (Python).</li>
      <li><span class="text-lime-700/80">Web mapping</span> and web development.</li>
      <li><span class="text-lime-700/80">VertiGIS Studio</span> and Workflow application development.</li>
      <li>Data visualisation and <span class="text-lime-700/80">dashboards</span></li>
      <li>Data processing, <span class="text-lime-700/80">automation</span>, Supervised machine-learning.</li>
      <li><span class="text-lime-700/80">CAD</span> software drafting (AutoCAD)</li>
    </ul>
  </div>
</div>

<style lang="postcss">
  .service-list li {
    display: block;
    text-indent: -1.6em;
    padding-left: 1.2em;
  }
  .service-list li:before {
    content: "✓ ";
    margin-right: 0.5em;
  }
</style>
